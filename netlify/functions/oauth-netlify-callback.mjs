...
const callbackPage=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>Connecting Netlify · WeaveRelay</title></head><body style="background:#0a0e12;color:#eef7f2;font-family:system-ui,sans-serif;padding:32px"><main><h1>Connecting Netlify…</h1><p id="status">Finishing secure authorization. You will return to WeaveRelay automatically.</p></main><script>(()=>{const status=document.querySelector('#status'),params=new URLSearchParams(location.hash.slice(1)),accessToken=params.get('access_token'),state=params.get('state'),oauthError=params.get('error_description')||params.get('error');const decodeOrigin=value=>{try{const part=String(value||'').split('.')[0],base64=part.replace(/-/g,'+').replace(/_/g,'/'),text=atob(base64.padEnd(Math.ceil(base64.length/4)*4,'=')),u=new URL(text),host=u.hostname.toLowerCase(),allowed=u.protocol==='https:'&&(u.origin==='https://weaverelay.com'||u.origin==='https://www.weaverelay.com'||/^deploy-preview-\\d+--weaverelay\\.netlify\\.app$/.test(host)||/^[a-z0-9-]+--weaverelay\\.netlify\\.app$/.test(host));return allowed?u.origin:''}catch{return ''}};const returnOrigin=decodeOrigin(state);history.replaceState(null,'',location.pathname);const fail=message=>{status.textContent='Netlify authorization did not finish. Returning to WeaveRelay…';const destination=returnOrigin||location.origin;location.replace(destination+'/app.html?oauth=netlify&error='+encodeURIComponent(message||'authorization_failed'))};if(oauthError||!accessToken||!state){fail(oauthError||'missing_callback_values');return}if(returnOrigin&&returnOrigin!==location.origin){const fragment=new URLSearchParams({access_token:accessToken,state}).toString();location.replace(returnOrigin+location.pathname+'#'+fragment);return}fetch(location.pathname,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({accessToken,state})}).then(async response=>{const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Netlify authorization failed.');location.replace('/app.html?w='+encodeURIComponent(data.workspaceId)+'&oauth=netlify&connected=1')}).catch(error=>fail(error.message))})();</script></body></html>`;
