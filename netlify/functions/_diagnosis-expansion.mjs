import { closestProviderFixLink } from './_provider-fix-links.mjs';
const clean=v=>String(v??'').trim();
const severityRank={critical:4,high:3,medium:2,low:1};
const byId=checks=>Object.fromEntries((checks||[]).map(c=>[c.id,c]));

function finding(id,severity,title,explanation,evidence,actions,provider,repair,evidenceMeta={}){return{id,severity,title,explanation,evidence:[evidence].filter(Boolean),actions,provider,repair:repair||{supported:false,approvalRequired:true,label:'GUIDED FIX'},openProvider:closestProviderFixLink(provider,evidenceMeta)}}
...
export function augmentDiagnosis(inputDiagnosis={},snapshot={}){
  ...
  for(const [id,provider,label] of [['vercel.live','vercel','Vercel'],['render.live','render','Render'],['cloudflare.live','cloudflare','Cloudflare'],['neon.live','neon','Neon'],['resend.live','resend','Resend']]){
    if(c[id]?.status==='FAIL')put(diagnosis.findings,finding(`${provider}-connection-failed`,'high',`${label} connection is failing`,c[id].detail,id,[`Reconnect ${label} with the minimum required permission, then rerun diagnosis.`],provider,{supported:true,approvalRequired:true,type:'reconnect-provider',provider,label:`Reconnect ${label}`},c[id].evidence));
  }

  const comfy=c['map.app-runpod-comfyui']||c['map.runpod-comfyui'];
  if(comfy?.status==='FAIL')put(diagnosis.findings,finding('runpod-comfyui-runtime-failed','critical','RunPod is present, but the ComfyUI runtime is not healthy',comfy.detail,comfy.id,['Inspect the proven RunPod target and ComfyUI runtime. WeaveRelay will not automatically start compute because that can create GPU spend. A separately approved start/restart repair can be added only when the exact target and cost boundary are proven.'],'runpod',{supported:false,approvalRequired:true,label:'REVIEW COMPUTE'},comfy.evidence));

  const workflow=c['comfyui.workflow-compatibility'];
  if(workflow?.status==='FAIL')put(diagnosis.findings,finding('comfyui-workflow-incompatible','high','The selected ComfyUI workflow does not match the live runtime',workflow.detail,workflow.id,['Install or select the missing model/node dependency, or point the app at the intended ComfyUI runtime. WeaveRelay will not install arbitrary code or models automatically.'],'comfyui',{supported:false,approvalRequired:true,label:'REPAIR WORKFLOW'},workflow.evidence));
