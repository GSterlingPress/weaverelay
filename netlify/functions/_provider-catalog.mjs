...
export function providerRecord(id,input={}){const definition=PROVIDERS[id];if(!definition)throw new Error(`Unsupported provider: ${id}`);const allowed=new Set(['not_connected','pending','detected','needs_action','connected','error','revoked']);const status=allowed.has(input.status)?input.status:'not_connected';return{id,label:definition.label,category:definition.category,authorization:definition.authorization,purpose:definition.purpose,status,detail:String(input.detail||''),checkedAt:input.checkedAt||null}}
export function connectionPlan(providers=[]){return providers.map(provider=>{if(provider.status==='connected')return{provider:provider.id,state:'done',next:'No action needed.'};if(provider.status==='detected')return{provider:provider.id,state:'verify',next:`Verify the existing ${provider.label} connection without replacing it.`};if(provider.status==='error')return{provider:provider.id,state:'repair',next:`Repair ${provider.label} using the least-privilege authorization method.`};return{provider:provider.id,state:'connect',next:`Authorize ${provider.label} with the minimum permissions needed for this workspace.`}})}
