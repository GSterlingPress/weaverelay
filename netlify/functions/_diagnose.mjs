...
const PROVIDERS={
  ...
  stripe:{label:'Stripe',url:'https://dashboard.stripe.com/'}
};
...
export function diagnoseSnapshot(input={}){
  ...
  if(c['github.live']?.status==='FAIL')findings.push(finding('github-unreachable','high','GitHub connection is failing',c['github.live'].detail,['github.live'],['Reconnect GitHub with repository access.'],'github'));
  if(c['supabase.live']?.status==='FAIL')findings.push(finding('supabase-unreachable','high','Supabase connection is failing',c['supabase.live'].detail,['supabase.live'],['Verify the Supabase Management API authorization.'],'supabase'));
  if(c['stripe.live']?.status==='FAIL')findings.push(finding('stripe-unreachable','high','Stripe connection is failing',c['stripe.live'].detail,['stripe.live'],['Reconnect Stripe with the required restricted read permission.'],'stripe'));
  ...
  add(findings,c['env.railway-config-coverage'],'railway-runtime-config','high','Railway runtime configuration needs attention',['Review the missing runtime configuration names before changing another provider.'],'railway',{supported:false,approvalRequired:true,label:'Repair Railway configuration'});
  add(findings,c['boundary.stripe-webhooks'],'stripe-webhook-boundary','high','Stripe webhook boundary needs attention',['Review Stripe webhook access/configuration for this production app.'],'stripe',{supported:false,approvalRequired:true,label:'Repair Stripe webhook'});
  add(findings,c['map.cross-system'],'cross-system-map-incomplete','medium','The cross-system map could not be completed',['Run diagnosis again and verify each provider is still readable.']);
