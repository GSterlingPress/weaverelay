...
import{diagnoseStripeHandlerFailure}from'./_stripe-handler-diagnosis.mjs';
...
export default async request=>{
  ...
  try{
    ...
    const seed=workspace.seedSnapshot||{product:workspace.name,topology:workspace.stackMap||{},checks:[]},checks=[...(seed.checks||[])],secrets={},now=new Date().toISOString();

    for(const provider of ['github','netlify','railway','supabase','stripe']){
      const c=await readConnection(workspace.id,provider).catch(()=>null);if(!c?.id||c.status==='revoked')continue;
      try{const secret=decryptSecret(await readSecret(c.id));secrets[provider]=secret.accessToken;let live;
        if(provider==='github'){const r=await fetch('https://api.github.com/user',{headers:{authorization:`Bearer ${secret.accessToken}`,accept:'application/vnd.github+json','user-agent':'weaverelay'}});await r.text();live={id:'github.live',label:'GitHub',status:r.ok?'PASS':'FAIL',detail:r.ok?'GitHub answered a live read-only account probe.':`GitHub returned HTTP ${r.status}.`,evidence:{source:'weaverelay-live-oauth',...(r.ok?{}:{httpStatus:r.status})}}}else live=checkForProvider(provider,await probeCredential(provider,secret.accessToken));
        upsert(checks,live);c.lastCheckedAt=now;c.status=live.status==='PASS'?'connected':'error';c.updatedAt=now;await writeConnection(workspace.id,provider,c)
    ...
    if(workspace.lastRepair?.type==='stripe-webhook-host'&&workspace.lastRepair?.configurationVerified===true){
      const boundary=checkById(checks,'map.railway-stripe-webhook'),boundaryVerified=boundary?.status==='PASS';
