...
export default async request=>{
  ...
  const inspect=request.clone(),execute=request.clone();
  try{
    const user=await requireUser(inspect),body=await inspect.json(),before=await requireWorkspace(user.id,body.workspaceId),previousSnapshot=before.lastDiagnosticSnapshot||null;
    const response=await expandedDiagnose(execute);if(!response.ok)return response;
