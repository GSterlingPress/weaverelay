...
const nonProductionContext=()=>['deploy-preview','branch-deploy','dev','dev-server'].includes(String(process.env.CONTEXT||process.env.DEPLOY_CONTEXT||'').toLowerCase());
...
async function normalizeContextEvidence(workspace,checks){const out=[];for(const check of checks){if(optionalNoData(check)){out.push({...check,status:'SKIPPED',detail:`${check.detail} This evidence source is optional and is not a current failure.`,evidence:{...(check.evidence||{}),optional:true,absenceIsFailure:false}});continue}const provider=CHECK_PROVIDER[check.id];if(!provider||!nonProductionContext()||check.status!=='WARN'){out.push(check);continue}const connection=await readConnection(workspace.id,provider).catch(()=>null);if(!connection?.id){out.push(check);continue}const encrypted=await readSecret(connection.id).catch(()=>null);if(!encrypted){out.push(check);continue}try{decryptSecret(encrypted);out.push(check)}catch{out.push({...check,status:'SKIPPED',detail:`${provider[0].toUpperCase()+provider.slice(1)} has a stored connection, but this non-production deploy cannot decrypt the credential from its owning deployment context. Provider health was not tested here.`,evidence:{...(check.evidence||{}),source:'weaverelay-cross-context',crossContextCredential:true,providerHealthTested:false,productionCredentialChanged:false}})}}return out}
...
export default async request=>{
  ...
  try{
    ...
    const normalized=await normalizeContextEvidence(workspace,(result.checks||[]).map(c=>stampCurrent(c,now)));
