...
const CHECK_PROVIDER={'github.live':'github','netlify.account':'netlify','railway.runtime':'railway','supabase.live':'supabase','stripe.live':'stripe','runpod.live':'runpod','vercel.live':'vercel','render.live':'render','cloudflare.live':'cloudflare','neon.live':'neon','resend.live':'resend'};
function requestContext(request){const env=String(process.env.CONTEXT||process.env.DEPLOY_CONTEXT||'').toLowerCase(),prime=String(process.env.DEPLOY_PRIME_URL||'').toLowerCase();let host='',forwarded='';try{host=new URL(request.url).hostname.toLowerCase()}catch{}forwarded=String(request.headers.get('x-forwarded-host')||request.headers.get('host')||'').split(',')[0].trim().toLowerCase();const signals={envDeployPreview:env==='deploy-preview',envBranchDeploy:env==='branch-deploy',envDev:['dev','dev-server'].includes(env),primeDeployPreview:/deploy-preview-\d+--/.test(prime),requestDeployPreview:/deploy-preview-\d+--weaverelay\.netlify\.app$/.test(host),forwardedDeployPreview:/deploy-preview-\d+--weaverelay\.netlify\.app$/.test(forwarded)};return{nonProduction:Object.values(signals).some(Boolean),signals}}
...
function optionalNoData(check){const d=String(check?.detail||'');return check?.id==='website.browser-runtime'&&/no recent privacy-safe browser runtime evidence/i.test(d)||check?.id==='website.synthetic-journey'&&/no recent approved synthetic journey result/i.test(d)}
async function normalizeContextEvidence(workspace,checks,context){const out=[];for(const check of checks){if(optionalNoData(check)){out.push({...check,status:'SKIPPED',detail:`${check.detail} This evidence source is optional and is not a current failure.`,evidence:{...(check.evidence||{}),optional:true,absenceIsFailure:false}});continue}const provider=CHECK_PROVIDER[check.id];if(!provider||!context.nonProduction||check.status!=='WARN'){out.push(check);continue}const connection=await readConnection(workspace.id,provider).catch(()=>null);if(!connection?.id){out.push(check);continue}const encrypted=await readSecret(connection.id).catch(()=>null);if(!encrypted){out.push(check);continue}try{decryptSecret(encrypted);out.push(check)}catch{out.push({...check,status:'SKIPPED',detail:`${provider[0].toUpperCase()+provider.slice(1)} has a stored connection, but this development deploy is isolated from the credential's owning context. Provider health was not tested here.`,evidence:{...(check.evidence||{}),source:'weaverelay-cross-context',crossContextCredential:true,providerHealthTested:false,productionCredentialChanged:false,previewIsolation:true,runtimeContextSignals:context.signals}})}}return out}
...
export default async request=>{
  ...
  const context=requestContext(request),inspect=request.clone(),execute=request.clone();let user=null,body=null,before=null,expandedCompleted=false;
  try{
    ...
    const normalized=await normalizeContextEvidence(workspace,(result.checks||[]).map(c=>stampCurrent(c,now)),context);
    const raw=sanitizeSnapshot({product:workspace.name,generatedAt:now,topology:result.stackMap||workspace.stackMap||{},checks:normalized});
    ...
    return json(200,{ok:true,workspaceId:workspace.id,diagnosis,checks:snapshot.checks,truthSummary:snapshot.truthSummary,stackMap:workspace.stackMap||snapshot.topology,runtimeContext:{classification:context.nonProduction?'non-production':'production-or-unknown',signals:context.signals}});
