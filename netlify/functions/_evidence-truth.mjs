...
function withTruth(check,{now}){const observed=evidenceTime(check),ageMs=observed==null?null:Math.max(0,now-observed),limit=MAX_AGE_MS[freshnessClass(check.id)]||MAX_AGE_MS.default,stale=explicitStale(check)||(ageMs!=null&&ageMs>limit),ageUnknown=observed==null,strength=sourceStrength(check),freshnessScore=stale?0:ageUnknown?.55:Math.max(.2,1-(ageMs/limit)*.65),confidence=Math.round(strength*freshnessScore);return{...check,evidence:{...(check.evidence||{}),stale,truth:{...(check.evidence?.truth||{}),observedAt:observed?new Date(observed).toISOString():null,ageMs,stale,ageUnknown,sourceStrength:strength,confidence,independenceKey:independenceKey(check)}}}}
function priorConflict(current,previous){if(!previous||current.status===previous.status||current.status==='WARN'||previous.status==='WARN'||current.status==='SKIPPED'||previous.status==='SKIPPED')return null;const currentTruth=current.evidence?.truth||{},previousStrength=sourceStrength(previous),previousObserved=evidenceTime(previous),currentObserved=evidenceTime(current),independent=independenceKey(current)!==independenceKey(previous),currentClearlyNewer=currentObserved!=null&&(previousObserved==null||currentObserved>previousObserved),strengthGap=currentTruth.sourceStrength-previousStrength;let decisive=false,winner='unresolved';if(currentClearlyNewer&&strengthGap>=-15){decisive=true;winner='current'}else if(previousStrength-currentTruth.sourceStrength>=25){decisive=true;winner='previous'}return{decisive,winner,independent,previousStatus:previous.status,previousStrength,previousObservedAt:previousObserved?new Date(previousObserved).toISOString():null}}
