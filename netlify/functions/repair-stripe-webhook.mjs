...
export default async request=>{
  ...
  try{
    ...
    const result=await applyStripeWebhookRepair({workspace,railwayToken,stripeToken});const now=new Date().toISOString();
    workspace.lastRepair={type:'stripe-webhook-host',provider:'stripe',changed:Boolean(result.changed),configurationVerified:Boolean(result.verified),runtimeVerified:false,targetHost:result.targetHost||null,pathPreserved:Boolean(result.pathPreserved),endpointUrlsRetained:false,signingSecretsRetained:false,approvedBy:user.id,approvedAt:now};
    workspace.updatedAt=now;await writeWorkspace(workspace);
    return json(200,{ok:true,repair:{type:'stripe-webhook-host',changed:Boolean(result.changed),configurationVerified:Boolean(result.verified),runtimeVerified:false,targetHost:result.targetHost||null,pathPreserved:Boolean(result.pathPreserved),endpointUrlsRetained:false,signingSecretsRetained:false},message:'Stripe webhook destination was corrected with a host-only change and the saved endpoint was verified. Run diagnosis again to verify the Stripe → Railway boundary.'});
