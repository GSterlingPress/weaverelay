...
export async function consumeLoginToken(token){const claims=readLoginToken(token);const usedKey=`used/${sha(claims.jti)}.json`;const existing=await usedLinks().get(usedKey,{type:'json',consistency:'strong'});if(existing)throw new Error('This sign-in link has already been used.');await usedLinks().setJSON(usedKey,{usedAt:new Date().toISOString(),expiresAt:new Date(claims.exp).toISOString()},{onlyIfNew:true});const userId=sha(claims.email).slice(0,32),now=new Date().toISOString();const current=await users().get(`user/${userId}.json`,{type:'json',consistency:'strong'});const user={id:userId,email:claims.email,createdAt:current?.createdAt||now,lastSignedInAt:now};await users().setJSON(`user/${userId}.json`,user);const raw=crypto.randomBytes(32).toString('base64url'),session={userId,email:claims.email,createdAt:now,expiresAt:new Date(Date.now()+SESSION_TTL_MS).toISOString()};await sessions().setJSON(`session/${sha(raw)}.json`,session);return{user,sessionToken:raw,next:safeNext(claims.next)};}
