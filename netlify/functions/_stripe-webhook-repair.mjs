...
async function railwayTargets(token,workspaceName,fetchImpl=fetch){
  const list=await graphql(token,'query { projects { edges { node { id name } } } }',{},fetchImpl);if(!list.ok)return[];const projects=(list.data?.projects?.edges||[]).map(e=>e?.node).filter(Boolean);const norm=v=>clean(v).toLowerCase().replace(/[^a-z0-9]+/g,'');const wanted=norm(workspaceName);let candidates=projects.filter(p=>{const n=norm(p.name);return wanted&&n&&(n.includes(wanted)||wanted.includes(n))});if(!candidates.length)candidates=projects.slice(0,6);else candidates=candidates.slice(0,4);const targets=[];
  for(const p of candidates){const q=await graphql(token,'query project($id: String!) { project(id: $id) { services { edges { node { id name } } } environments { edges { node { id name } } } } }',{id:p.id},fetchImpl);const pr=q.data?.project;if(!q.ok||!pr)continue;const envs=(pr.environments?.edges||[]).map(e=>e?.node).filter(Boolean).sort((a,b)=>/^(production|prod)$/i.test(b.name)-/^(production|prod)$/i.test(a.name)).slice(0,2);const services=(pr.services?.edges||[]).map(e=>e?.node).filter(Boolean).slice(0,10);for(const env of envs)for(const service of services){const v=await graphql(token,'query variables($projectId: String!, $environmentId: String!, $serviceId: String) { variables(projectId: $projectId, environmentId: $environmentId, serviceId: $serviceId) }',{projectId:p.id,environmentId:env.id,serviceId:service.id},fetchImpl);const domain=clean(v.data?.variables?.RAILWAY_PUBLIC_DOMAIN).toLowerCase().replace(/^https?:\/\//,'').split('/')[0];if(domain)targets.push({domain,projectId:p.id,projectName:clean(p.name),environmentId:env.id,environmentName:clean(env.name),serviceId:service.id,serviceName:clean(service.name)})}}
  const byDomain=new Map();for(const t of targets)if(!byDomain.has(t.domain))byDomain.set(t.domain,t);return[...byDomain.values()]
}
...
export async function inspectStripeWebhookRepair({workspace,railwayToken,stripeToken,fetchImpl=fetch}={}){
  ...
  const [appHosts,ownedTargets,wh]=await Promise.all([scanAppRailwayHosts(workspace?.siteOrigin,fetchImpl),railwayTargets(railwayToken,workspace?.name,fetchImpl),stripeGet(stripeToken,'/v1/webhook_endpoints?limit=100',fetchImpl)]);
  ...
  const matchedTargets=ownedTargets.filter(t=>appHosts.includes(t.domain));
  if(matchedTargets.length!==1)return{eligible:false,reason:'railway-target-not-unique',provenTargetCount:matchedTargets.length};
  if(enabled.length!==1)return{eligible:false,reason:'stripe-endpoint-not-unique',enabledEndpointCount:enabled.length};
  const endpoint=enabled[0],target=matchedTargets[0],currentHost=hostOf(endpoint?.url),targetHost=target.domain;
  if(!currentHost||!safeWebhookPath(endpoint?.url))return{eligible:false,reason:'webhook-route-not-proven'};
  if(currentHost===targetHost)return{eligible:false,reason:'already-correct',alreadyCorrect:true,targetHost,target};
  return{eligible:true,reason:'single-proven-host-mismatch',targetHost,target,endpointId:clean(endpoint.id),currentHost,pathPreserved:true};
}
...
export async function applyStripeWebhookRepair({workspace,railwayToken,stripeToken,fetchImpl=fetch}={}){
  ...
  return{changed:true,verified:true,targetHost:proposal.targetHost,target:proposal.target,pathPreserved:true,endpointId:proposal.endpointId,endpointUrlsRetained:false,signingSecretsRetained:false};
}
