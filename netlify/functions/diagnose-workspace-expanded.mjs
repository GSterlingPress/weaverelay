...
export default async request=>{
  ...
  try{
    ...
    for(const provider of EXPANDED){
      ...
      try{
        const secret=decryptSecret(await readSecret(connection.id)),probe=await probeCredential(provider,secret?.accessToken),live=checkForProvider(provider,probe);upsert(checks,live);connection.lastCheckedAt=now;connection.status=live.status==='PASS'?'connected':'error';connection.updatedAt=now;connection.lastErrorCode=live.status==='PASS'?null:'probe_failed';await writeConnection(workspace.id,provider,connection);workspace.providers=(workspace.providers||[]).map(p=>p.id===provider?{...p,status:connection.status,detail:live.detail,checkedAt:now}:p);changed=true;
      }catch{
        upsert(checks,{id:`${provider}.live`,label:provider[0].toUpperCase()+provider.slice(1),status:'WARN',detail:`Live ${provider} probe could not complete.`,evidence:{source:'weaverelay-live-expanded',resourceBodiesRetained:false}});changed=true;
      }
    }
    if(changed){const seed=workspace.lastDiagnosticSnapshot||workspace.seedSnapshot||{product:workspace.name,topology:workspace.stackMap||{}};const snapshot=sanitizeSnapshot({...seed,product:workspace.name,generatedAt:now,checks}),diagnosis=diagnoseSnapshot(snapshot);workspace.lastDiagnosticSnapshot=snapshot;workspace.diagnosis=diagnosis;workspace.status=diagnosis.status==='healthy'?'ready':'needs_action';workspace.updatedAt=now;await writeWorkspace(workspace);return new Response(JSON.stringify({ok:true,workspaceId:workspace.id,diagnosis,checks:snapshot.checks,stackMap:workspace.stackMap||snapshot.topology}),{status:200,headers:{'content-type':'application/json','cache-control':'no-store','x-content-type-options':'nosniff'}})}
    return base;
  }catch{return base}
};
