...
import { sanitizeClientSnapshotShape } from './_client-snapshot-boundary.mjs';
...
export default async request=>{
  ...
  const snapshot=sanitizeSnapshot(sanitizeClientSnapshotShape(rawSnapshot)),id=workspaceId(clientId,siteOrigin),existing=await readWorkspace(id).catch(()=>null),incoming=body.providerStatus&&typeof body.providerStatus==='object'?body.providerStatus:{},now=new Date().toISOString();
