...
def _annotate_after_commit(result: dict, registry: SourceRegistry, contract: str, invoice: str, evidence: list[str]) -> None:
    c = registry.doc_id_for_path(contract); i = registry.doc_id_for_path(invoice)
...
def run_zero_dave_audit(*, contract: str, invoice: str, field: str | None = None, evidence: list[str] | None = None, original_names: dict[str, str] | None = None) -> dict:
    evidence = list(evidence or []); original_names = original_names or {}; audit_id = _audit_id()
    ...
    for analyzer in ANALYZERS:
        initial_results[analyzer] = run_sealed_process(analyzer, audit_id=audit_id, contract=contract, invoice=invoice, field=field, evidence=evidence)
    for r in initial_results.values(): _annotate_after_commit(r, registry, contract, invoice, evidence)
    initial_consensus = build_consensus(initial_results)

    reconciliation = run_clue_free_reanalysis(
        audit_id=audit_id, contract=contract, invoice=invoice, field=field, evidence=evidence,
        initial_results=initial_results, initial_consensus=initial_consensus,
    )
    analyzer_results = reconciliation['final_analyzer_results']
    for r in analyzer_results.values(): _annotate_after_commit(r, registry, contract, invoice, evidence)
    consensus = reconciliation['final_consensus']
    ...
    return {
        ...
        'source_registry':registry.manifest(),'analyzer_catalogs':analyzer_catalogs,
        'analyzer_results':analyzer_results,'consensus':consensus,'reanalysis':reconciliation,
        ...
        'independence':{
            'sealed_room_enforced':True,'process_level_isolation':True,'same_original_evidence_only':True,'independent_financial_engines':True,
            'peer_outputs_hidden_until_all_committed':True,'peer_catalogs_hidden_until_all_committed':True,'disagreement_clues_forbidden':True,
            'reanalysis_from_original_evidence_only':True,'reanalysis_reruns_all_three_to_avoid_dissenter_leakage':True,
            'consensus_is_only_cross_analyzer_reader':True,'shared_deterministic_engine_kernel':False,'customer_triple_independent_badge_allowed':True,
            'engines':{'A':'validated deterministic Audit kernel','B':'independent contract-first parser/calculator','C':'independent ledger-first evidence reconstruction + sentence-window contract scorer'},
            'note':'A/B/C execute as separate OS child processes in disposable sealed evidence rooms. Reanalysis launches fresh rooms/processes for all three from original evidence only; no peer clue is supplied.'
        }
    }
