...
def render(j,s,error=None):
    ...
    body+="<button class='btn' type='submit'>Confirm / correct types</button></form>"
    ready=(pending==0 and any(d['classification']['type'] in CONTRACT for d in docs) and any(d['classification']['type'] in INVOICE for d in docs))
    if ready:body+=f"<form method='post' action='/ingest/{html.escape(j)}/audit' style='margin-top:16px'><label><input type='checkbox' name='authorization' value='yes' required> I am authorized to analyze these documents.</label><br><br><button class='btn'>Run audit →</button></form>"
    else:body+="<p class='muted'>Automatic routing requires at least one contract/rate document, one prime contractor invoice, and no unresolved classifications.</p>"
...
def register_v11(app):
    ...
    async def ingest(request:Request,documents:list[UploadFile]=File(...)):
        ...
        try:
            ...
            shutil.rmtree(w,ignore_errors=True);return page('V11',f"<div class=c>{html.escape(str(e))}</div>")
    ...
    async def review(request:Request,job_id:str):
        ...
        save(job_id,s);return render(job_id,s)
    ...
    async def run(request:Request,job_id:str,authorization:str=Form(...)):
        ...
        try:
            ...
            return page('V11 Audit Complete',f"<div class='h'><h1>Audit complete.</h1><p>{len(docs)} documents were classified and routed.</p></div><div class='c'><h2>${total:,.2f} potential overpayment</h2><a class='btn' href='/report/{html.escape(job_id)}/json'>Download audit JSON</a></div>")
